...
#' @export
#'
make_qc_plots <- function(pep_data               = pep_data,
								  			  pg_data                = pg_data,
   ...
   if(peak_shape             == T) {
      ...
      peak_width_plot <- peak_width_data %>%
         ...
         ggplot2::ggplot(ggplot2::aes(x = rt,
																			y = peak_width)) +
         ggplot2::stat_summary_bin(ggplot2::aes(col = sample),
																	 size     = 1,
																	 geom     = 'line',
																	 binwidth = 1,
																	 fun      = median) +
         ggplot2::labs(title = 'Median Peak Width Over Retention Time',
											 x     = 'Retention Time [min]',
											 y     = 'Median Peak Width [min]',
											 color = 'Sample') +
         ggplot2::scale_color_manual(
					 values = NatParksPalettes::natparks.pals('Denali',
   ...
   if(charge_state           == T) {
      charge_state_data <- pep_data %>%
         ...
         dplyr::group_by(sample,
												 charge) %>%
         dplyr::summarise(charge_per = n/total_peptides * 100) %>%
         dplyr::ungroup() %>%
         dplyr::mutate(charge := forcats::fct_inorder(factor(charge))) %>%
         dplyr::mutate(sample := factor(sample,
																				levels = dplyr::unique(
																					stringr::str_sort(sample,
																														numeric = T)
      ...
      charge_state_plot <- charge_state_data %>%
         ...
         ggplot2::geom_col(col  = 'black',
													 size = 1) +
         ggplot2::geom_text(data = charge_state_data %>%
																dplyr::filter(charge_per < 5),
            ggplot2::aes(label = round(charge_per,
																			 digits = 1)),
            position = ggplot2::position_stack(vjust = 0.9)) +
         ggplot2::labs(title    = 'Charge Distribution',
   ...
   if(cv                     == T) {
      cv_data <- pg_data %>%
         dplyr::distinct(protein_group,
												 condition,
												 log2_pg_maxlfq) %>%
         dplyr::rename('intensity' = log2_pg_maxlfq) %>%
         ...
         dplyr::group_by(condition, protein_group) %>%
         ...
         dplyr::distinct(condition,
												 ...
												 cv) %>%
         tidyr::drop_na() %>%
         tidyr::pivot_longer(cols = starts_with('cv'),
														 names_to  = 'type',
														 values_to = 'values') %>%
         dplyr::mutate(type = ifelse(type == 'cv', 
																		 condition, 
																		 'combined')) %>%
         dplyr::mutate(type = forcats::fct_relevel(as.factor(type),
																									 'combined')) %>%
         dplyr::select(-condition) %>%
         dplyr::group_by(type) %>%
         dplyr::mutate(median = stats::median(values)) %>%
         dplyr::distinct() %>%
         dplyr::filter(values < 200)

      cv_plot <- ggplot2::ggplot(cv_data,
																 ggplot2::aes(x    = type,
																							y    = values,
																							fill = type)) +
         ...
         ggplot2::geom_boxplot(width = 0.15,
															 ...
															 na.rm = T,
															 alpha = 0.6) +
         ggplot2::labs(title = 'Protein Group Coefficients of Variation',
											 x     = '',
   ...
   if(precursor_coverage     == T) {
      ...
      precursor_count_plot <- precursor_count %>%
         dplyr::mutate(sample := factor(sample,
																				levels = unique(stringr::str_sort(sample,
																																					numeric = T)))) %>%
         ggplot2::ggplot(ggplot2::aes(x    = sample,
																			y    = count,
																			fill = condition)) +
         ggplot2::geom_col(col  = 'black',
													 size = 1) +
         ggplot2::labs(title = 'Number of Precursors Quantified',
            					 x     = '',
            					 y     = 'Count',
            					 fill  = 'Condition') +
         plot_theme +
   ...
   if(peptide_coverage       == T) {
      ...
      peptide_count_plot <- peptide_count%>%
         ...
         ggplot2::geom_col(col  = 'black',
													 size = 1) +
         ggplot2::labs(title = 'Number of Peptides Quantified',
											 x     = '',
											 y     = 'Count',
											 fill  = 'Condition') +
         plot_theme +
   ...
   if(protein_coverage       == T) {
      ...
      pg_count_plot <- pg_count %>%
         ...
         ggplot2::geom_col(col  = 'black',
													 size = 1) +
         ggplot2::labs(title = 'Number of Protein Groups Quantified',
											 x     = '',
											 y     = 'Count',
											 fill  = 'Condition') +
         plot_theme +
   ...
   if(precursor_completeness == T) {
      precursor_completeness_data <- pep_data %>%
         ...
         dplyr::group_by(sample) %>%
         dplyr::summarise(
					 completeness = sum(!is.na(log2_precursor_normalized))/dplyr::n() * 100,
           .groups      = 'drop') %>%
         dplyr::left_join(y = pep_data %>%
															dplyr::distinct(sample,
																							condition),
													by = 'sample') %>%
      ...
      precursor_completeness_plot <- precursor_completeness_data %>%
         ...
         ggplot2::ggplot(ggplot2::aes(x    = sample,
																			y    = completeness,
																			fill = condition)) +
         ggplot2::geom_col(fill = NatParksPalettes::natparks.pals('Denali', 
																																	1),
													 col  = 'black',
													 size = 1) +
         ggplot2::geom_text(data = precursor_completeness_data,
														ggplot2::aes(label = round(completeness,
																											 digits = 1)),
														position = ggplot2::position_stack(vjust = 0.5)) +
         ggplot2::scale_y_continuous(limits = c(0, 
																								100)) +
         ggplot2::labs(title = 'Precursor-level Data Completeness',
											 x     = '',
											 y     = 'Data Completeness [%]') +
         plot_theme +
         ggplot2::scale_fill_manual(
   ...
   if(peptide_completeness   == T) {
      ...
      peptide_completeness_plot <- peptide_completeness_data %>%
         ...
         ggplot2::geom_col(col  = 'black',
													 size = 1) +
         ggplot2::geom_text(data = peptide_completeness_data,
														ggplot2::aes(label = round(completeness,
																											 digits = 1)),
														position = ggplot2::position_stack(vjust = 0.5)) +
         ggplot2::scale_y_continuous(limits = c(0, 
																								100)) +
         ggplot2::labs(title = 'Peptide-level Data Completeness',
											 x     = '',
											 y     = 'Data Completeness [%]') +
         plot_theme +
         ggplot2::scale_fill_manual(
   ...
   if(protein_completeness   == T) {
      pg_completeness_data <- pep_data %>%
         ...
         dplyr::group_by(sample) %>%
         dplyr::summarise(
					 completeness = sum(!is.na(log2_pg_maxlfq))/dplyr::n() * 100,
					 .groups      = 'drop')  %>%
         dplyr::left_join(y = pep_data %>%
															dplyr::distinct(sample,
																							condition),
      ...
      pg_completeness_plot <- pg_completeness_data %>%
         ...
         ggplot2::geom_col(col  = 'black',
													 size = 1) +
         ggplot2::geom_text(data = pg_completeness_data,
														ggplot2::aes(label = round(completeness,
																											 digits = 1)),
														position = ggplot2::position_stack(vjust = 0.5)) +
         ggplot2::scale_y_continuous(limits = c(0, 
																								100)) +
         ggplot2::labs(title = 'Protein Group-level Data Completeness',
											 x     = '',
											 y     = 'Data Completeness [%]') +
         plot_theme +
         ggplot2::scale_fill_manual(
   ...
   if(precursor_intensity    == T) {
         ...
         ggplot2::geom_boxplot(width = 0.15,
															 ...
															 na.rm = T,
															 alpha = 0.6) +
         ggplot2::labs(title = 'Precursor-level Run Normalized Intensities',
											 x     = '',
   ...
   if(protein_intensity      == T) {
      ...
      pg_intensity_plot <- pg_intensity_data %>%
         ...
         ggplot2::geom_boxplot(width = 0.15,
															 ...
															 na.rm = T,
															 alpha = 0.6) +
         ggplot2::labs(title = 'Protein Group-level Run MaxLFQ Abundances',
											 x     = '',
											 y     = 'Log2 MaxLFQ') +
         ggplot2::scale_fill_manual(
            values = NatParksPalettes::natparks.pals('Denali',
