pep_count_fun <- function(df) {
   ...
   as.data.frame(table(unique(df[,
                                 c('protein_group',
                                   'modified_peptide')])$'protein_group'))
}
...
#' @export
#'
annotate_protein <- function(data = data,
   ...
   pep_count <- data %>%
      ...
      tidyr::nest() %>%
      dplyr::mutate(n_peptides = purrr::map(data, 
                                            pep_count_fun)) %>%
      dplyr::select(-c('data')) %>%
   ...
   imputed_with_names <- dplyr::bind_cols(
      ...
      data %>%
         ...
         dplyr::select(c(gene_name,
                         protein_name))) %>%
      dplyr::relocate(c(gene_name:protein_name),
                      .after = protein_group) %>%
      tidyr::pivot_longer(cols = 4:(3 + dplyr::n_distinct(sample_annotation)),
                          names_to = 'sample',
                          values_to = 'log2_pg_maxlfq') %>%
      dplyr::relocate(sample,
                      .before = protein_group) %>%
      dplyr::left_join(y = sample_annotation,
                       by = 'sample') %>%
