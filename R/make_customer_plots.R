...
#'
make_customer_plots <- function(pep_data = pep_data,
                                pg_data = pg_data,
   ...
   if(sample_correlation   == T) {
      ...
      conditions <- unique(dplyr::pull(annotation,
                                       condition))

      n_conditions <- length(conditions)

      conditions_colours <- NatParksPalettes::natparks.pals('Denali')[1:n_conditions]
      ...
      sample_heatmap <-
         pheatmap::pheatmap(correlation,
                            ...
                            main = "Correlation based hierachical clustering of samples",
                             color = NatParksPalettes::natparks.pals('Volcanoes', 
                                                                     100, 
                                                                     'continuous'),
                            treeheight_row = 0)
   }
   if(protein_coverage     == T) {
      ...
      pg_count_plot <- pg_count_data %>%
         ...
         ggplot2::labs(title = 'Number of Protein Groups Quantified',
                       ...
                       fill  = 'Condition') +
         plot_theme +
         ggplot2::scale_fill_manual(values = NatParksPalettes::natparks.pals(
            'Denali',
   ...
   if(protein_completeness == T) {
      pg_completeness_data <- pep_data %>%
         ...
         tidyr::complete(sample,
                         protein_group) %>%
         dplyr::group_by(sample) %>%
         dplyr::summarise(completeness = sum(!is.na(log2_pg_maxlfq))/dplyr::n() * 100,
                          .groups      = 'drop') %>%
   ...
   if(volcano              == T) {
      ...
      cutoff_line <- volcano_data %>%
         dplyr::mutate(mean_cutoff = -log10(mean_cutoff)) %>%
      ...
      vol_plot <- volcano_data %>%
         dplyr::filter(!((abs(diff) > 1) &
                         ifelse(is.na(pval < .data$mean_cutoff),
                                FALSE,
                                pval < .data$mean_cutoff))) %>%
         ggplot2::ggplot(ggplot2::aes(label1 = NULL,
                                      label2 = protein_group)) +
         ggplot2::geom_point(ggplot2::aes(x = diff,
                                          y = -log10(pval)),
                             colour = '#9da7bf') +
         ggplot2::geom_point(data = dplyr::filter(volcano_data,
         ...
         ggplot2::geom_hline(data = cutoff_line,
                             ggplot2::aes(yintercept = .data$mean_cutoff),
                             linetype = 'dashed') +
         ggplot2::geom_vline(xintercept = fold_change_cutoff,
                             linetype   = 'dashed') +
         ggplot2::geom_vline(xintercept = -fold_change_cutoff,
                             linetype   = 'dashed') +
         ggplot2::theme_bw() +
         ...
         ggplot2::scale_x_continuous(
            breaks = seq(round(-1 * max(abs(volcano_data %>%
                                               dplyr::select(diff)), 
                                        na.rm = T) - 0.5,
                               0),
                         round(max(abs(volcano_data %>%
                                          dplyr::select(diff)), 
                                   na.rm = T) + 0.5, 
                               0),
                         1)
         ...
         ggforce::facet_wrap_paginate(~ comparison, 
                                      ncol = 1, 
                                      nrow = 1)
   }
   ...
   if(volcano                == T) {
      for(i in 1:ggforce::n_pages(vol_plot)) {
         vol_save <- vol_plot + ggforce::facet_wrap_paginate(~ comparison, 
                                                             ncol = 1, 
                                                             nrow = 1, 
                                                             page = i)
         print(vol_save)
