...
#' @importFrom magrittr %>%
#'
prepare_data <- function(filepath     = 'report.tsv',
   ...
   message('[3/4] Calculating MaxLFQ intensities ...
          ...
          ', appendLF = F)
   data_maxlfq <- diann::diann_maxlfq(data_filtered,
   ...
   data_tidy <- data_maxlfq %>%
      ...
      tibble::as_tibble() %>%
      tidyr::pivot_longer(cols      = 2:(ncol(data_maxlfq) + 1),
                          names_to  = 'File.Name',
                          values_to = 'PG.MaxLFQ') %>%
      dplyr::mutate(Log2.PG.Max.LFQ = log2(PG.MaxLFQ)) %>%
      dplyr::right_join(y = data_in %>%
                              dplyr::select(-c('PG.Quantity':'Genes.MaxLFQ.Unique')),
                        by = c('File.Name',
                               'Protein.Group')) %>%
      janitor::clean_names() %>%
      dplyr::mutate(peak_width = rt_stop - rt_start) %>%
      ...
      dplyr::mutate('log2_precursor_normalized' = log2(precursor_normalised),
                    .after = precursor_normalised) %>%
      dplyr::rename('pg_maxlfq'            = pg_max_lfq,
      ...
      dplyr::relocate(protein_name,
                      .before = modified_peptide) %>%
      dplyr::relocate(c(pg_maxlfq:log2_pg_maxlfq),
                      .after = log2_precursor_normalized) %>%
      dplyr::relocate(peak_width,
                      .after = charge) %>%
      dplyr::relocate(rt,
                      .after = peak_width) %>%
