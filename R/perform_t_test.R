...
#' @importFrom rlang :=
#'
perform_t_test <- function(pg_data = pg_data) {
   ## Assign Missing-ness ------------------------------------------------------
   message('[1/4] Assigning missing-ness ...', appendLF = F)
   ### Create all pair-wise comparisons -----------------------------------------
   all_conditions <- dplyr::unique(dplyr::pull(pg_data,
                                               condition))

   #Create a data frame that contains all combinations to be tested
   all_combinations <- tibble::as_tibble(t(utils::combn(all_conditions,
                                                        m = 2)),
                                         .name_repair = ~ make.names(.,
                                                                     unique = T)) %>%
      dplyr::rename(V1 = X,
                    V2 = X.1) %>%
      dplyr::mutate(combinations = paste0(V1,
                                          '_vs_',
                                          V2)) %>% 
      ...
      dplyr::mutate(comparison = list(combinations)) %>%
   ...
   data_prep <- pg_data %>%
      ...
      dplyr::distinct(sample,
                      ...
                      protein_group,
                      log2_pg_maxlfq) %>%
      tidyr::complete(tidyr::nesting(sample,
                                     condition),
                      protein_group) %>%
      dplyr::group_by(protein_group,
                      condition) %>%
      dplyr::mutate(n_detect = sum(!is.na(log2_pg_maxlfq))) %>%
      dplyr::mutate(n_replicates = dplyr::n()) %>%
      dplyr::ungroup() %>%
      dplyr::left_join(all_combinations,
                       by = 'condition') %>%
   ...
   unequal_replicates <- data_prep %>%
      ...
      dplyr::distinct(n_replicates,
                      comparison
      ) %>%
      dplyr::group_by(comparison) %>%
      dplyr::mutate(n = dplyr::n()) %>%
      dplyr::filter(n > 1) %>%
      dplyr::mutate(n_replicates = paste0(n_replicates,
                                          collapse = '/')) %>%
      dplyr::distinct(n_replicates,
                      comparison)
   ...
   if (nrow(unequal_replicates) != 0) {
      ...
      message(paste0(utils::capture.output(unequal_replicates),
                     collapse = "\n"))
   }

   ### Assign missing values ---------------------------------------------------
   comparison_data <- data_prep %>%
      dplyr::mutate(type = ifelse(condition == stringr::str_extract(comparison,
                                                                    pattern = '(?<=_vs_).+'),
                                  'control',
                                  'experimental')) %>%
      split(comparison) %>%
      purrr::map_df(.f = ~ .x %>%
                        tidyr::pivot_wider(names_from = type,
                                           values_from = c(n_detect,
                                                           n_replicates)) %>%
            dplyr::group_by(protein_group) %>%
            tidyr::fill(n_detect_experimental,
                        n_detect_control,
                        n_replicates_experimental,
                        n_replicates_control,
                        .direction = 'updown') %>%
            dplyr::ungroup() %>%
            dplyr::mutate(missingness = dplyr::case_when(
                  n_detect_control == n_replicates_control &
                        n_detect_experimental == n_replicates_experimental ~ 'complete',
                  n_detect_control <= floor(n_replicates_control * 0.2) &
                        n_detect_experimental == n_replicates_experimental ~ 'MNAR',
                  n_detect_control == n_replicates_control &
                        n_detect_experimental <= floor(n_replicates_experimental * 0.2) ~ 'MNAR',
                  n_detect_control >= max(floor(n_replicates_control * 0.7), 
                                          1) &
                        n_detect_experimental >= max(floor(n_replicates_control * 0.7),
                                                     1) ~ 'MAR'
            ))) %>%
      dplyr::select(-c(n_detect_control,
                       n_detect_experimental,
                       n_replicates_control,
                       n_replicates_experimental)) %>%
      dplyr::arrange(factor(protein_group,
                            levels = dplyr::unique(stringr::str_sort(protein_group),
                                                   numeric = T)))
   ...
   moderated_t_test_input <- comparison_data %>%
      ...
      tibble::column_to_rownames(var = rlang::as_name('protein_group')) %>%
      as.matrix()

   ### Defining the t-test design ----------------------------------------------
   moderated_t_test_map <- comparison_data %>%
      dplyr::distinct(sample,condition) %>%
      dplyr::mutate(condition := paste0('x', 
                                        condition)) %>%
      dplyr::arrange(sample)

   moderated_t_test_design <- stats::model.matrix(
         ~ 0 + factor(stringr::str_replace_all(dplyr::pull(moderated_t_test_map,
                                                           condition),
                                               pattern = ' ',
                                               replacement = '_'))
         )
   ...
   colnames(moderated_t_test_design) <- levels(
         factor(stringr::str_replace_all(dplyr::pull(moderated_t_test_map,
                                                     condition),
                                         pattern = ' ',
                                         replacement = '_')))
   message('DONE', appendLF = F)
   ...
   moderated_t_test_fit <- suppressWarnings(limma::lmFit(moderated_t_test_input,
                                                         moderated_t_test_design))

   ### Create a matrix of differences ------------------------------------------
   names <- paste0('x',
                   stringr::str_extract(unique(dplyr::pull(comparison_data,
                                                           comparison)),
                                        pattern = '.+(?=_vs_)'), 
                   '_vs_x',
                   stringr::str_extract(unique( dplyr::pull(comparison_data, 
   ...
   comparisons <- paste0(
         'x',
         stringr::str_extract(stringr::str_replace_all(unique(dplyr::pull(comparison_data,
                                                                          comparison)),
                                                       pattern = ' ',
                                                       replacement = '_'),
                              pattern = '.+(?=_vs_)'),
   ...
   contrast_matrix <- eval(rlang::expr(limma::makeContrasts(!!!unlist(combinations),
                                                            levels = moderated_t_test_design)))

   ### Compute Contrasts From Regression Model ---------------------------------
   moderated_t_test_fit2 <- limma::contrasts.fit(moderated_t_test_fit,
                                                 contrast_matrix)
   ...
   moderated_t_test_result <- purrr::map_dfr(
      ...
      dplyr::mutate(comparison = stringr::str_replace_all(comparison,
                                                          pattern = '^x|(?<=_vs_)x',
                                                          replacement = ' ')) %>%
      dplyr::rename(diff          = logFC,
                    CI_2.5        = CI.L,
                    CI_97.5       = CI.R,
                    t_statistic   = t,
                    avg_abundance = AveExpr,
                    pval          = P.Value) %>%
      dplyr::left_join(moderated_test_missingness,
                       by = c('protein_group',
                              'comparison')) %>%
      dplyr::select(-c(adj.P.Val,
                       t_statistic,
                       B,
                       missingness,
                       n_obs))
